...
* **Net GMV (Gross Merchandise Value):** The actual revenue generated.
    * Note: in the exercise document, **Gross Margin Value** is mentioned, but since we don't have COGS, we will use Net GMV as a proxy for revenue.
...
To validate our substitution logic, we constructed a specific categorization for `dim_model`. The challenge was to distinguish **Direct Substitution** (The product set to replace the target) from **Indirect Substitution** (product that might be indirectly affected by the suppression of the Target), and **Noise** (products that will be used as a control group).
